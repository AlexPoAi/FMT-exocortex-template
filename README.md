...
| **memory/** | Canonical protocol source в шаблоне: `protocol-open.md`, `protocol-work.md`, `protocol-close.md` + оперативная память. Эти файлы потом зеркалятся в runtime memory |
...
### Источник истины для протоколов

- **Canonical source:** `FMT-exocortex-template/memory/protocol-open.md`, `protocol-work.md`, `protocol-close.md`
- **Runtime mirror:** `~/.claude/projects/<slug>/memory/`
- **Backup snapshot:** `DS-strategy/exocortex/`

Это три разных слоя. Template memory — авторский источник. Runtime mirror — установленная копия для Claude Code. Backup snapshot — слепок после close-flow, а не место редактирования.
