...
| **T4: Созидание (IWE)** | + AI CLI, Стратег, Git, свои базы знаний | **Это руководство** |
...
- **AI CLI** (`Codex` / `Claude`) — ИИ-помощник, который знает твои цели, задачи и методологию. Помнит контекст между сессиями
...
|------|-----|-------|---------------------|
| **0** | Подготовка (Git, Node, AI CLI) | 15-20 мин | **обязательно** |
...
Если у тебя уже установлены Git, Node.js, GitHub CLI и хотя бы один AI CLI (`codex` или `claude`) — переходи к [Этапу 1](#этап-1-установка-iwe-5-мин).
...
Node.js — среда выполнения JavaScript. Нужна для установки части AI CLI-инструментов и dev tooling. npm — менеджер пакетов Node.js (устанавливается вместе с Node.js).
...
### 0.5 AI CLI для агентного слоя

Для локального агентного слоя нужен AI CLI, который умеет работать в терминале, читать файлы, выполнять команды и писать изменения в workspace.

На текущем состоянии экосистемы рекомендуется:

- `Codex` как primary provider для локальных агентов
- `Claude` как fallback-provider

### 0.5a Codex CLI (рекомендуется)

Codex CLI — рабочий non-interactive и interactive provider для локальных агентов.

### 0.5b Claude Code CLI (опционально как fallback)

Claude Code остаётся поддержанным provider'ом и полезен как fallback-path или для отдельных пользовательских сценариев.
...
При первом запуске нужный CLI попросит пройти login/auth flow — следуй инструкциям в терминале.
...
VS Code — редактор кода с графическим интерфейсом. В нём удобно работать с AI CLI: видишь файлы, терминал и агентный контур в одном окне. **Без VS Code** придётся работать только через терминал — это возможно, но менее наглядно.

- Скачай и установи: [code.visualstudio.com](https://code.visualstudio.com/)
- Дальше можно работать либо через встроенный терминал (`codex` / `claude`), либо через совместимое расширение по своему provider-path
...
4. **Обновление `memory/MEMORY.md`** — Твои рабочие продукты появятся в таблице
...
- `memory/MEMORY.md` — ваша оперативная память (РП, уроки)
...
Нет, не обязательно. Для IWE нужен **хотя бы один agent-capable AI CLI**. На текущем состоянии экосистемы рекомендуется начать с `Codex` как primary path. Подписка Anthropic полезна как fallback-path и для Claude-compatible сценариев, но больше не является обязательным условием для базового запуска.
...
**Какие есть agent-capable альтернативы?**

| Альтернатива | Что это | Цена | Модели |
|---|---|---|---|
| **Codex CLI** | CLI-агент в терминале | зависит от плана OpenAI | GPT |
...
Да. Стратег — это автоматизация (утренние планы, ревью). Без него AI CLI + CLAUDE.md + memory/ работают полностью. Планируешь вручную.
...
**Как проверить MCP?**
Открой AI CLI в папке экзокортекса и проверь, что knowledge-path доступен. Для Claude-compatible маршрута можно набрать `/mcp` — серверы должны быть Connected. Если серверов нет — добавь их через https://claude.ai/settings/connectors (см. шаг 1.3b).

**Безопасны ли мои данные?**
DS-strategy — приватный репо. `memory/MEMORY.md` — локальный файл. Ничего не публикуется без твоего ведома. Подробности о том, что отправляется на внешние серверы (Claude API, WakaTime, GitHub) — см. раздел [Безопасность и приватность](#безопасность-и-приватность).
...
| Когда | Что | Как |
|-------|-----|-----|
| После первой недели | Пройди сессию стратегирования (Пн) | Агент сам предложит |
| Через 2 недели | Создай первый Pack (личная база знаний) | `codex` или `claude` → «Помоги создать мой первый Pack» |
