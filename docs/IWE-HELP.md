...
Для Claude-compatible маршрута MCP подключается через https://claude.ai/settings/connectors (см. SETUP-GUIDE §1.3b). Проверка: `/mcp` в Claude-compatible клиенте → серверы Connected.
...
**Основной AI CLI не запускается** — проверь, что нужный CLI установлен и отвечает на `codex --version` или `claude --version`. Если используешь Claude как fallback-provider — отдельно проверь login/subscription.
...
**MCP не работает** — проверь, что в используемом Claude-compatible маршруте серверы подключены и видны в `/mcp`. Если их нет — добавь через https://claude.ai/settings/connectors (см. SETUP-GUIDE §1.3b).
