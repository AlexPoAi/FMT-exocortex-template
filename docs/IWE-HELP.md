...
| Вид | Что | Примеры |
|-----|-----|---------|
| **Системы** | Программы с 4D-границами | AI CLI (Codex / Claude), Telegram-бот, MCP-серверы, WakaTime, Git, экзокортекс (файлы), Neon DB |
| **Описания** | Знания, загружаемые в системы | FPF/SPF/ZP, Pack-сущности, промпты ролей, содержимое экзокортекса |
| **Роли** | Функция, не исполнитель | Стратег (R1) ← AI CLI holder (`Codex` / `Claude`), Экстрактор (R2), Синхронизатор (R8), Пользователь ← Человек |
...
- Node.js v18+ и npm
- AI CLI для локального агентного слоя
  - рекомендуется `Codex`
  - `Claude Code` можно держать как fallback-provider

### Опционально
- VS Code (рекомендуется) или любой другой редактор с терминалом. AI CLI работает в любом терминале (Terminal.app, iTerm2 и др.). VS Code удобен: редактор + терминал + AI CLI в одном окне
...
MCP (Model Context Protocol) — протокол, через который основной AI CLI подключается к базе знаний платформы. Два сервера:
...
Для Claude-compatible маршрута MCP подключается через https://claude.ai/settings/connectors (см. SETUP-GUIDE §1.3b). Если работаешь через такой клиент, проверка: `/mcp` → серверы Connected.
...
Каждая сессия в AI CLI — три стадии:
...
**Основной AI CLI не запускается** — проверь, что нужный CLI установлен и отвечает на `codex --version` или `claude --version`. Если используешь Claude как fallback-provider — отдельно проверь login/subscription. Если `Codex` и `Claude` установлены оба, operational primary выбирается policy/runtime arbitration слоем.
...
**MCP не работает** — проверь, что в используемом Claude-compatible маршруте серверы подключены и видны в `/mcp`. Если их нет — добавь через https://claude.ai/settings/connectors (см. SETUP-GUIDE §1.3b). В остальных AI CLI-маршрутах смотри их собственный способ подключения knowledge-path.
...
| MCP | Model Context Protocol — доступ AI CLI к внешним базам знаний |
