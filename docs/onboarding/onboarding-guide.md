...
| Планы не работают | **Ритуалы ОРЗ + AI CLI** | Утром — план дня (автоматически). Вечером — итоги. Каждую неделю — ревью. ИИ не даёт забыть незавершённое |
| ИИ не помогает | **AI CLI + Экзокортекс** | ИИ читает ТВОИ файлы, знает ТВОИ цели, помнит ТВОЮ историю. Это персональный ассистент, а не обезличенный чат-бот |
...
1. Установить AI CLI (`Codex` рекомендуется, `Claude Code` можно как fallback)
2. Создать аккаунт на GitHub (бесплатно)
3. Запустить одну команду, которая настроит всё остальное

> **ИИ поможет.** Если у тебя уже стоит основной AI CLI — просто скажи ему: «Помоги мне установить IWE». Он проведёт тебя через каждый шаг.
>
> Подробная инструкция: [SETUP-GUIDE.md](../SETUP-GUIDE.md)

**Что нужно (минимум):**
- Компьютер (Mac, Linux или Windows с WSL)
- Подписка или логин в выбранный AI CLI
- GitHub аккаунт (бесплатно)

### Шаг 3. Первая стратегическая сессия (~30 минут)

После установки ты запускаешь AI CLI и проводишь первую сессию:
...
- **Терминал** — окно, в которое ты вводишь команды текстом (AI CLI подскажет, что вводить)
- **CLI** — просто способ общения с компьютером через текст вместо кнопок

После установки ты будешь взаимодействовать в основном с AI CLI — на обычном русском языке.
...
|---------|----------------|---------------|
| **T1 — Старт** | AI CLI + экзокортекс | Персональный ИИ-ассистент, который тебя помнит |
...
В центре IWE — **ты**. Ты принимаешь решения, мыслишь, направляешь. AI CLI — твой напарник: он усиливает, структурирует, берёт на себя рутину. Но решения — всегда за тобой.
...
**AI CLI** — это ИИ-ассистент, который работает прямо в терминале. Он не просто отвечает на вопросы — он читает твои файлы, помогает планировать, напоминает о незавершённых делах, предлагает структуру. Это твой напарник, а не поисковик.
...
**VS Code** — бесплатный редактор от Microsoft. На старте тебе он не нужен — AI CLI работает в терминале, и ты общаешься с ним текстом. VS Code понадобится позже, когда захочешь самостоятельно просматривать и редактировать файлы. Думай о нём как о «продвинутом блокноте» для своих текстов и планов.

### Знания

**Экзокортекс** — твоя вторая память. Набор файлов, которые ИИ-ассистент (AI CLI) читает и обновляет. Когда ты начинаешь работу — он знает, на чём ты остановился вчера. Когда заканчиваешь — записывает выводы. Ты больше не теряешь контекст между сессиями.
...
| **AI CLI** | ИИ-напарник: усиливает, структурирует, берёт рутину. Работает по тем же принципам | Инженер-помощник |
