...
> **Scope:** этот skill относится к `.claude` slash-command слою. Если текущий агент работает не в Claude-среде или slash-команды недоступны, НЕ пытайся вызывать `/run-protocol`; прочитай соответствующий `memory/protocol-*.md` и выполни шаги вручную.
