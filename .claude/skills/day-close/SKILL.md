...
```bash
for repo in $(ls /Users/alexander/IWE/); do
  if [ -d /Users/alexander/IWE/$repo/.git ]; then
    commits=$(git -C /Users/alexander/IWE/$repo log --since="today 00:00" --oneline --no-merges 2>/dev/null)
...
grep -nE "→ ждёт|ждёт|dep:|блокер|blocked:|остановлен|ждёт согласования" \
  /Users/alexander/.claude/projects/*/memory/MEMORY.md 2>/dev/null
```
...
> **Условный шаг:** скрипт опционален. Если `/Users/alexander/Github/{{GOVERNANCE_REPO}}/scripts/check-index-health.py` отсутствует — пропустить.

```bash
SCRIPT="/Users/alexander/Github/{{GOVERNANCE_REPO}}/scripts/check-index-health.py"
