...
> **Runtime contract:** протокол provider-agnostic. Его можно выполнять в текущем аутентифицированном агенте (`Codex`, `Claude`, совместимый CLI). Если `Claude` не залогинен, но `Codex` доступен, Close НЕ блокируется и продолжается в `Codex`. Команда `claude /login` нужна только для Claude-specific route, а не как глобальное условие закрытия.
...
> **Исполнение:** prefer `/run-protocol close` (пошаговый чеклист, предотвращает пропуск шагов). Если slash-route или Claude runtime недоступны, выполнить этот же чеклист вручную в текущем аутентифицированном агенте. Truthful blocker здесь только один: нет ни одного рабочего аутентифицированного runtime.
...
3. **KE (Knowledge Extraction)** → прочитай и выполни `FMT-exocortex-template/roles/extractor/prompts/session-close.md`:
...
> **Исполнение:** prefer `/run-protocol day-close` (пошаговый чеклист). Если slash-route, Claude-auth или cloud-runtime недоступны, Day Close всё равно выполняется по этому же чеклисту вручную в текущем аутентифицированном агенте. Для `Codex` это канонический safe-route, а не деградация. `claude /login` требуется только если пользователь хочет вернуть именно Claude provider.
...
#### 6b. Канонический day-close summary (обязательный формат)

После завершения Day Close агент обязан собрать один human-readable итог дня. Этот итог должен быть одинакового класса для всех агентов и не может подменяться token-report или техническим usage-отчётом.

Обязательные блоки summary:

1. **Состояние экзокортекса**
   - общий verdict `🟢 / 🟡 / 🔴`
   - кратко: что живо, что stale, что сломано

2. **Какие агенты отработали**
   - strategist / synchronizer / extractor / health-check
   - по каждому: `сработал / не сработал / частично`

3. **Что сделано за день**
   - главные результаты
   - только существенные артефакты и закрытые РП

4. **Что в работе**
   - активные РП и незавершённые контуры

5. **Где требуется внимание пользователя**
   - блокеры
   - решения, которые агент не может принять сам
   - stale/critical зоны
...
> **Запрещено:** выдавать вместо day-close summary только token-report, cost report или иной технический хвост.
