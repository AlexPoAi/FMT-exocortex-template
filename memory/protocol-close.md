...
> **Исполнение:** основной путь — выполнить этот чеклист вручную в текущем аутентифицированном агенте. `/run-protocol close` — только optional convenience layer для Claude-среды, где slash-skill реально доступен. Truthful blocker здесь только один: нет ни одного рабочего аутентифицированного runtime.
...
> **Исполнение:** основной путь — выполнить этот чеклист вручную в текущем аутентифицированном агенте. `/run-protocol day-close` — только optional convenience layer для Claude-среды со slash-skill. Для `Codex` manual execution по этому файлу — канонический safe-route, а не деградация. `claude /login` требуется только если пользователь хочет вернуть именно Claude provider.
