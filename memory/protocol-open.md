...
### Стартовый экран экзокортекса

0. Сначала показать обязательный экран открытия сессии с состоянием «мозга экзокортекса».
1. Если экран красный — обычное открытие дня запрещено, сначала сообщить о проблеме и нужном действии.
2. Если экран жёлтый — продолжать только после явного подтверждения пользователя.
3. Только после экрана переходить к WP Gate.
...
**Шаг 1.** Показать стартовый экран экзокортекса и назвать режим среды: зелёный / жёлтый / красный.
...
- Для смены модели: перезапустить Claude с флагом `--model <name>`
- Алиасы (если настроены в ~/.zshrc):
  - `claude-haiku` → запуск с Haiku
  - `claude-sonnet` → запуск с Sonnet (default)
  - `claude-opus` → запуск с Opus
- VSCode настройка: `claudeCode.selectedModel` в settings.json (применяется при новом запуске)

**Шаг 3.** Дождаться согласования пользователя на открытие сессии и работу (включая модель и, при жёлтом статусе, разрешение продолжать в деградации).

**Шаг 4.** Определить файлы/репо, ограничения. Если есть context file (`DS-my-strategy/inbox/WP-{N}*.md`) — прочитать.
