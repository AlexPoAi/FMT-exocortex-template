...
## Поведенческий контракт ИИ-ролей

Для ролей `type: agential` каждый prompt/runner должен сохранять disciplined loop:
1. **Plan**: явные допущения, критерий успеха, минимальный scope.
2. **Act**: выполнить только подтверждённый bounded step.
3. **Verify**: проверить результат заранее названным способом.
4. **Adjust**: исправить в том же scope или остановиться, если нужен новый scope.

Роль не должна молча расширять задачу, создавать артефакты "на будущее", трогать соседние dirty changes или хранить секреты без явного безопасного канала.
