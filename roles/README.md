...
## Общая дисциплина ролей

Для ИИ-ролей действует лёгкий Karpathy-style loop:
- перед нетривиальной работой назвать допущения, критерий успеха и минимальный scope;
- не добавлять speculative work и новые сущности без подтверждённой необходимости;
- работать коротким циклом `plan -> act -> verify -> adjust`;
- если меняется scope, права доступа или появляется риск затронуть чужие dirty changes — остановиться и пересогласовать.
