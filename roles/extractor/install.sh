#!/bin/bash
# Extractor: установка launchd-агентов inbox-check + session-watcher + obsidian-fleeting-intake
# inbox-check: каждые 3 часа
# session-watcher: каждые 5 минут
# obsidian-fleeting-intake: каждые 24 часа + RunAtLoad
set -e
...
# Resolve source paths (Generated runtime -> workspace fallback -> FMT legacy)
...
# Ensure scripts are executable (runtime path)
[ -f "$EXTRACTOR_TARGET" ] && chmod +x "$EXTRACTOR_TARGET"
[ -f "$WATCHER_TARGET" ] && chmod +x "$WATCHER_TARGET"
[ -f "$INTAKE_TARGET" ] && chmod +x "$INTAKE_TARGET"
...
echo "  ✓ Session-watcher interval: every 5 minutes"
...
echo "Uninstall: launchctl unload \$PLIST_DIR/com.extractor.*.plist && rm \$PLIST_DIR/com.extractor.*.plist"
